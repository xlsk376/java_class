...
public class 배열3_알고리즘_인풋쇼핑 {
	...
	public static void main(String[] args) {
		 ...
		 */
		String [] nameList = {"새우깡" , "감자깡" , "고래밥"}; // 상품이름
		int [] priceList = {1000, 2100, 3300}; // 가격
		int [] countList = {3,1,1}; // 상품개수
				
		int [] input = {1,2,2,0,0,2,1,1}; // 주문순서
			
		int[] sel = {0, 0, 0};
		for(int i=0; i<input.length; i++) {
			if(countList[input[i]] > 0) { // 과자 재고 확인
				System.out.println(nameList[input[i]] + "를 구입합니다.");
				countList[input[i]] -= 1; // 현재 있는 과자 재고에서 -1
				sel[input[i]] += 1; // 팔린과자 개수 체크
			}else {
